...
def CT(data):
    ...
    data_ct = np.zeros(data.shape)

    for i in range(data.shape[0]):#循环样品数量次，对每一个样品，行
        MEAN[i] = np.mean(data[i])#对每一个样品求均值
        data_ct[i,:] = data[i,:]-MEAN[i]
    return data_ct
...
def wave(data):
    ...
    def wave_(data):
        ...
        coeffs = pywt.wavedec(data, 'db8', level=maxlev)#小波分解
        threshold = 0.04
        for i in range(1, len(coeffs)):#coeffs第一个np数组不变。改变后两个数组的值
            coeffs[i] = pywt.threshold(coeffs[i], threshold * np.max(coeffs[i]))
        datarec = pywt.waverec(coeffs, 'db8')#小波重构
        return datarec
